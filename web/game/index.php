...
/* Starting session. */
	session_start();
		...
		if(!isset($_SESSION['user_id']) || (trim($_SESSION['user_id']) == '')) {
			...
			header("location: ".$_SESSION['PAGE_ROOT']);
			exit();
		}
		/* auth */
		...
		if (isset($_GET['id']))
			$sess_game_id = $_GET['id'];
		$sess_redirect_to = $_SESSION['redirect_to'];
		$sess_user_id = $_SESSION['user_id'];

		$_SESSION['game_id'] = $sess_game_id;
...
/* Include necessary scripts. */
	...
	include_once('../plugins/config_loader.php');
...
/* Check if game exists with given ID. */
	if (!is_dir('game_folders/'.$sess_game_id)) {
...
/* Check if actual user is an invited gamer of the given game. */
	if (!is_file('game_folders/'.$sess_game_id.'/user_'.$sess_user_id.'.txt')) {
		$hva->add_alert( $l_config['game_not_participant'].'--'.$sess_user_id );
		header( 'Location: '.$sess_redirect_to );
		exit();
	}
...
/* Collecting data about the game from the database. */
	$db = new MysqlHandler();
	$db->start();
		$query = "SELECT * FROM games WHERE gamesID = '".$sess_game_id."'";
	...
	session_start();
		$_SESSION['game_title'] = $row['title'];
			$sess_game_title = $row['title'];
		$_SESSION['game_owner_id'] = $row['usersID'];
			$sess_game_owner_id = $row['usersID'];
		$_SESSION['game_desc'] = $row['description'];
		$sess_game_desc = $row['description'];
		$_SESSION['game_type'] = $row['type'];
		$_SESSION['game_created'] = $row['started'];

		/* Get game type name. */
			$ini_source = '../charsheet_xml/installed_games_list_'.$sess_user_id.'.ini';
			...
			$_SESSION['game_type_name'] = $game_type[0];
				$sess_game_type_name = $game_type[0];
			$_SESSION['game_type_full_name'] = $game_type[1];
				$sess_game_type_full_name = $game_type[1];

		/* Checking if actual user is the GM of the given game? */
			if (is_file('game_folders/'.$sess_game_id.'/gm.txt')) {
				$f = fopen('game_folders/'.$sess_game_id.'/gm.txt', 'r');
					$gm_id = fgets($f, 4096);
				fclose($f);
				if ($gm_id==$sess_user_id)
					$_SESSION['act_user_is_gm']=true;
				else
					$_SESSION['act_user_is_gm']=false;
				$sess_act_user_is_gm = $_SESSION['act_user_is_gm'];
			}
	session_write_close();
	
/* Loading charsheet information. */
	$charsheet_info_src = '../charsheet_xml/'.$sess_game_type_name.'/lang_'.$sess_game_type_name;
	$charsheet_info = config_loader($charsheet_info_src);
	
/* Loading charsheet configuration. */
	$conf_ini_url = '../charsheet_xml/'.$sess_game_type_name.'/conf_'.$sess_game_type_name;
...
/* Preloading character sheet for better performance. */
	...
	$xml_src = '../charsheet_xml/'.$sess_game_type_name.'/'.$sess_game_type_name.'.xml';
	if (!is_file($xml_src)) {
		$xml_any = glob('../charsheet_xml/'.$sess_game_type_name.'/*.xml');
...
<head>
	...
	<title>RPGCraft - '<?php echo $sess_game_title; ?>'</title>
	...
	<!-- STYLES -->
		...
		<?php
			/* Searching for stylesheets provided by the character sheet. */
				$css_src = '../charsheet_xml/'.$sess_game_type_name.'/'.$sess_game_type_name.'.css';
				if ( !is_file( $css_src ) ) {
					$any_css = glob('../charsheet_xml/'.$sess_game_type_name.'/*.css');
	...
	<!-- VARIABLES -->
		<script type='text/javascript'>
			var gm_id = <?php echo $sess_game_owner_id; ?>;
			var game_id = <?php echo $sess_game_id; ?>;
			var act_user_id = <?php echo $sess_user_id; ?>;
			var act_user_is_gm = <?php echo ($sess_act_user_is_gm)?'true':'false'; ?>;
...
<body>
	...
	<div id='show_entry_wrapper'>
		<div class='title'<?php if ($sess_act_user_is_gm) echo ' contenteditable="true"'; ?>></div>
		<div class='desc'<?php if ($sess_act_user_is_gm) echo ' contenteditable="true"'; ?>></div>
		<?php if ($sess_act_user_is_gm){ ?>
	...
	<div id='allWrapper' class=''>
		<div id='info'>
			<div onclick='show_desc("game_desc");' class='game_desc_short'><?php echo $sess_game_title; ?></div>
			<div onclick='show_desc("charsheet_desc");' class='charsheet_desc_short'><?php echo $sess_game_type_full_name; ?></div>
			<div class='dragger'></div>
		</div>
		<div id='desc_container'>
			<div class='game_desc desc_cont'>
				<?php echo $l_config['game_desc_title']; ?><br/><br/>
				<?php echo $sess_game_desc; ?></div>
		...
		<div class='eventsWrapper' id='eventsWrapper'>
			<div class='map_container'>
				...
				<?php
					if ($sess_act_user_is_gm) echo ' - <span onclick=\'add_new_map();\'>'.$l_config['gm_new_map'].'</span>';
				?>
				</div>
			...
			<div class='entry_container'>
				...
				<?php
					if ($sess_act_user_is_gm) echo ' - <span onclick=\'add_new_event_toggle();\'>'.$l_config['gm_new_event'].'</span>';
				?>
				</div>
		...
		<div id='character_sheet' class='<?php echo $sess_game_type_name.'_border_style'; ?>'>
			<div class='x'>X</div>
			<div class='dragger'></div>
			<div class='wrapper <?php echo $sess_game_type_name.'_bg_style'; ?>' id='charsheet_wrapper'>
