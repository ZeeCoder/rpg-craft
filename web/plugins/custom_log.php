<?php
	...
	*/
	if (strlen(session_id())==0) session_start();
		$c_sess_user_id = $_SESSION['user_id'];
	session_write_close();
	...
	class custom_log {
		...
		function __construct($type=0) {
			if ($type==0) $type= "rpg_user_log_".$c_sess_user_id;
			else if ($type==1) $type= "rpg_game_log_".$c_sess_user_id;
			else if ($type==2) $type= "client_check_users_".$c_sess_user_id;
			else if ($type==3) $type= "client_update_user_".$c_sess_user_id;
